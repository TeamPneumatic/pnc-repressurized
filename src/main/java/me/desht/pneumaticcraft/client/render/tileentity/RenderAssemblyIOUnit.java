...
public class RenderAssemblyIOUnit extends AbstractTileModelRenderer<TileEntityAssemblyIOUnit> {
    ...
    void renderModel(TileEntityAssemblyIOUnit te, float partialTicks) {
        if (te != null) {
            ...
            model.renderModel(0.0625F, renderAngles, MathHelper.lerp(partialTicks, te.oldClawProgress, te.clawProgress), te.getPrimaryInventory().getStackInSlot(0));
            renderManager.options.fancyGraphics = fancySetting;
        } else {
            model.renderModel(0.0625F, new float[]{0, 0, 35, 55, 0}, 0, ItemStack.EMPTY);
        }
    }
}
