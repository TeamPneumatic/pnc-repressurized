...
public class ThermostatModuleScreen extends AbstractTubeModuleScreen<ThermostatModule> {
    ...
    public void init() {
        ...
        advancedMode.setTooltip(List.of(xlate("pneumaticcraft.gui.tubeModule.advancedConfig.tooltip")));
    ...
    public void drawForeground(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        hLine(matrixStack, graphLeft + 4, graphRight, graphHighY + (graphLowY - graphHighY) * (15 - module.getLevel()) / 15, 0xFFFF0000);
    ...
    private void renderGraph(PoseStack matrixStack) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (longer) {
                ...
                font.draw(matrixStack, txt, graphLeft - 5 - font.width(txt), graphHighY + (graphLowY - graphHighY) * (15 - i) / 15f - 3, 0xFF303030);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            font.draw(matrixStack, txt, graphLeft + (graphRight - graphLeft) * offset / 100f - font.width(txt) / 2f + adjusts[i],
                graphLowY + 6, 0xFF303030);
            vLine(matrixStack, graphLeft + (graphRight - graphLeft) * offset / 100, graphHighY, graphLowY - 2, 0x33000000);
            vLine(matrixStack, graphLeft + (graphRight - graphLeft) * offset / 100, graphLowY - 5, graphLowY + 3, 0xFF303030);
        }
    }
