...
@ZenRegister
public class PressureChamber implements IRecipeManager<PressureChamberRecipe> {
    @ZenCodeType.Method
    ...
    public void remove(IIngredient output) {
        CraftTweakerAPI.apply(new ActionRemoveRecipe<>(this, iRecipe -> {
            if (iRecipe != null) {
                return iRecipe.value().getResultsForDisplay(ClientUtils.getClientLevel().registryAccess()).stream()
