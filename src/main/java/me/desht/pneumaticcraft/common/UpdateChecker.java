...
public class UpdateChecker extends Thread {
    ...
    private String incVersion(String version, int digit) {
        ...
        StringBuilder newVersion = new StringBuilder(Integer.toString(nums[0]));
        for (int i = 1; i < nums.length; i++) {
            newVersion.append(".").append(nums[i]);
        }
            ...
            if (astring.length > 0 && astring[0].equalsIgnoreCase("changelog")) {
                if (astring.length > 1) {
                    if (astring[1].equalsIgnoreCase("additions")) {
                        ...
                        ConfigHandler.general.enableUpdateChecker = true;
                        ConfigHandler.sync();
                        ...
                        ConfigHandler.general.enableUpdateChecker = false;
                        ConfigHandler.sync();
...
//                        ConfigHandler.enableUpdateChecker = false;
                    } else {
