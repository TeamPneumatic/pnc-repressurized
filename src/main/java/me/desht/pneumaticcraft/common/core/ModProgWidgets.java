...
public class ModProgWidgets {
    public static final DeferredRegister<ProgWidgetType<?>> PROG_WIDGETS = new DeferredRegister<>(ModRegistries.PROG_WIDGETS, Names.MOD_ID);
    ...
    public static final List<Supplier<? extends ProgWidgetType<?>>> WIDGET_LIST = new ArrayList<>();
    ...
    private static <C extends IProgWidget, T extends ProgWidgetType<C>> RegistryObject<T> register(String name, Supplier<C> sup) {
        ProgWidgetType<? extends IProgWidget> type = new ProgWidgetType<>(sup);
        WIDGET_LIST.add(() -> type);
        //noinspection unchecked
        return PROG_WIDGETS.register(name, () -> (T) type);
    }

    public static void registerCustom(ProgWidgetType type) {
        WIDGET_LIST.add(() -> type);
    }
}
