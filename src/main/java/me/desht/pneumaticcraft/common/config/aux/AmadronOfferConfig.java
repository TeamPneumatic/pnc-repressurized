...
public abstract class AmadronOfferConfig extends AuxConfigJson {
    ...
    protected final void readFromJson(JsonObject json) {
        ...
        for (JsonElement element : array) {
            AmadronOffer offer = ((JsonObject) element).has("inStock") ?
                    AmadronOfferCustom.fromJson((JsonObject) element) :
                    AmadronOffer.fromJson((JsonObject) element);
            if (offer != null) {
                offers.add(offer);
            }
        }
