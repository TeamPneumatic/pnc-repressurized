...
public class DroneAILiquidImport extends DroneAIBlockInteraction{
            ...
            for(int i = 0; i < 6; i++) {
                if(((ISidedWidget)widget).getSides()[i]) {
                    ...
                    if(importedFluid != null && ((ILiquidFiltered)widget).isFluidValid(importedFluid.getFluid())) {
                        ...
                        if(filledAmount > 0) {
                            if(!simulate) {
                                drone.getTank().fill(tank.drain(ForgeDirection.getOrientation(i), filledAmount, true), true);
                            }
                            return true;
                        }
                    }
