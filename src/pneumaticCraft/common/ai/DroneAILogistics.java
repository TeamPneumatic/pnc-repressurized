...
public class DroneAILogistics extends DroneAIBlockInteraction{
                        ...
                        if(provider.shouldProvideTo(priority)) {
                            ...
                            if(drone.getInventory().getStackInSlot(0) != null) {
                                ...
                                if(requestedAmount > 0) {
                                    providingStack = drone.getInventory().getStackInSlot(0).copy();
                                    providingStack.stackSize = requestedAmount;
                                    curAI = new DroneEntityAIInventoryExport(drone, new FakeWidgetItemLogistics(requester.getPos(), providingStack));
                                }
                            } else {
                                providingStack = tryProvide(provider, requester);
                            }
                            if(curAI != null) {
                                if(curAI.shouldExecute()) {
                                    requester.informIncomingStack(providingStack);
                                    requestingBlock = requester;
                                    transportingStack = providingStack;
                                    return true;
                                }
                            }
                        }
    ...
    protected boolean doBlockInteraction(ChunkPosition pos, double distToBlock){
        if(!curAI.continueExecuting()) {
            ...
            requestingBlock.informIncomingStack(transportingStack);
            return true;
        }
    }

    private ItemStack tryProvide(SemiBlockLogistics provider, SemiBlockLogistics requester){
        ...
        for(int i = 0; i < providingInventory.getSizeInventory(); i++) {
            ...
            if(providingStack != null && IOHelper.canExtractItemFromInventory(providingInventory, providingStack, i, 0)) {
                int requestedAmount = getRequestedAmount(requester, providingStack);
                if(requestedAmount > 0) {
                    providingStack = providingStack.copy();
                    providingStack.stackSize = requestedAmount;
                    curAI = new DroneEntityAIInventoryImport(drone, new FakeWidgetItemLogistics(provider.getPos(), providingStack));
    ...
    private int getRequestedAmount(SemiBlockLogistics requester, ItemStack providingStack){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            remainder = IOHelper.insert(requester.getTileEntity(), remainder, d, true);
    ...
    private static class FakeWidgetItemLogistics extends ProgWidgetAreaItemBase implements ISidedWidget, ICountWidget{
        private final ItemStack stack;
        private final Set<ChunkPosition> area;

        public FakeWidgetItemLogistics(ChunkPosition pos, ItemStack stack){
            this.stack = stack;
