...
public class ContainerPressureChamberInterface extends Container{
    TileEntityPressureChamberInterface te;

    public ContainerPressureChamberInterface(InventoryPlayer inventoryPlayer,
            TileEntityPressureChamberInterface teInterface){
        te = teInterface;

        // add the transfer slot.
        addSlotToContainer(new SlotUntouchable(teInterface, 0, 66, 35));

        // add the upgrade slots
        for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 2; j++) {
                addSlotToContainer(new SlotUpgrade(teInterface, i * 2 + j + 1, 20 + j * 18, 26 + i * 18));
            }
        }
        // add the export filter slots
        for(int i = 0; i < 3; i++) {
            for(int j = 0; j < 3; j++) {
                addSlotToContainer(new SlotPhantomUnstackable(teInterface, i * 3 + j + 5, 115 + j * 18, 25 + i * 18));
            }
        }
