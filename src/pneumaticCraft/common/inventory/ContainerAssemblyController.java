...
public class ContainerAssemblyController extends Container{
    TileEntityAssemblyController te;

    public ContainerAssemblyController(InventoryPlayer inventoryPlayer, TileEntityAssemblyController teAirCompressor){
        te = teAirCompressor;
        te.openGUI();
        // Add the burn slot.
        addSlotToContainer(new SlotItemSpecific(teAirCompressor, Itemss.assemblyProgram, 0, 74, 38));

        // add the upgrade slots
        for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 2; j++) {
                addSlotToContainer(new SlotUpgrade(teAirCompressor, i * 2 + j + 1, 13 + j * 18, 31 + i * 18));
            }
        }
