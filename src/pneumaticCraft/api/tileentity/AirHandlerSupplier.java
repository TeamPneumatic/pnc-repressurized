...
public class AirHandlerSupplier{
    ...
    public static IAirHandler getAirHandler(float dangerPressure, float criticalPressure, float maxFlow, float volume){
        ...
        try {
            if(airHandlerConstructor == null) airHandlerConstructor = Class.forName("pneumaticCraft.common.tileentity.TileEntityPneumaticBase").getConstructor(float.class, float.class, float.class, float.class);
            airHandler = (IAirHandler)airHandlerConstructor.newInstance(dangerPressure, criticalPressure, maxFlow, volume);
        } catch(Exception e) {
